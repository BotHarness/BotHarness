...
- The Computer sidebar's Running card now follows the AgentScreen pattern: a framed capture that reveals a blue **Open** pill on hover, and a fullscreen viewer whose title bar carries the Bot name, live status, stop, and collapse — Escape collapses, the page scroll is locked, and the resting card unmounts while the overlay is up so only one viewer WebSocket exists at a time. Entering shows a connecting state, sustained silence becomes an explicit no-picture state with retry, and all chrome colours read DSH design tokens or primitives so light and dark themes both render correctly ([#167](https://github.com/BotHarness/BotHarness/issues/167)).
...
- The multiline Channel composer now moves attachment and send controls into a dedicated bottom footer instead of vertically centering the attachment control beside tall input text ([#148](https://github.com/BotHarness/BotHarness/issues/148)).

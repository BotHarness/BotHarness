...
- The Computer viewer now syncs with Selkies' own session state instead of trusting canvas size alone: liveness requires a sized surface, changing pixels, and a quiet `#status-display` line — a static-but-healthy desktop still goes live after a short grace, and a stuck stream ages to the retry state — so opening the viewer no longer reports connected while Selkies is still connecting ([#221](https://github.com/BotHarness/BotHarness/issues/221)).

...
- Added an optional Computer plugin: a profile-scoped shared Linux desktop that runs locally in Docker, appears in the DSH Web Client as an authenticated VNC panel, requires an explicit authorization before start, streams image-pull progress, and stops itself when idle; PersonaBot binding and disk-location choice remain follow-up slices ([#150](https://github.com/BotHarness/BotHarness/issues/150)).

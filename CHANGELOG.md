...
- PersonaBot activity now follows the Host's revisioned Session projection in the pinned grid, Channel rows, and DM composer; the same state returns after a Host restart without relying on the local send indicator. PersonaBot avatars in the pinned grid, Channel rows, rail, and composer keep their activity indicator and media motion without a redundant outer outline; DM rows also omit the right-edge activity dot. DM sends keep new messages visible when following the timeline and preserve the scroll position while reading history ([#120](https://github.com/BotHarness/BotHarness/issues/120)).

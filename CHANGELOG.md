...
- PersonaBot activity now follows the Host's revisioned Session projection in the pinned grid, Channel rows, and DM composer; the same state returns after a Host restart without relying on the local send indicator ([#120](https://github.com/BotHarness/BotHarness/issues/120)).

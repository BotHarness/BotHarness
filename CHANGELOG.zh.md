...
- Human 现在可在 PersonaBot DM 侧栏授权或撤销已有 DSH 工作区；新事项会持久记录所选授权、主要工作目录与 `workspace-write`／`ask` 权限快照。撤销会阻止后续事项请求与原生 Session 恢复，已在运行的 turn 仍需 Human 手动停止（[#116](https://github.com/BotHarness/BotHarness/issues/116)）。

...
- 应用侧边栏的 Bot 模式切换按钮改为「通高的 Bot 标记 + 圆角标签块」两段式：再次点击整行会退出 Bot 模式，hover 时标签块右侧出现设置齿轮，点击直接打开设置对话框的 Bot 分区（[#177](https://github.com/BotHarness/BotHarness/issues/177)）。

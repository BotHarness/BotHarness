...
- PersonaBot 的活动状态现在由 Host 带修订号的 Session 投影同步到置顶网格、Channel 行与 DM 输入区；Host 重启后会从 Session 记录重建同一状态，不再依赖本地发送标志；置顶网格、Channel 行、侧栏及输入区的 PersonaBot 头像保留活动提示与本体动效，移除重复的外围描边；DM 行还移除了右侧活动圆点。发送 DM 时，停留在最新消息会保持新消息完整可见，浏览历史则保留滚动位置（[#120](https://github.com/BotHarness/BotHarness/issues/120)）。

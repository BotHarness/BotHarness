...
- PersonaBot 的活动状态现在由 Host 带修订号的 Session 投影同步到置顶网格、Channel 行与 DM 输入区；Host 重启后会从 Session 记录重建同一状态，不再依赖本地发送标志（[#120](https://github.com/BotHarness/BotHarness/issues/120)）。

...
export function WorkspaceGrantsEntry({ botSlug, actions, t }: ChannelSidebarEntryProps): ReactElement {
  ...
  const activeIds = new Set(grants.filter((grant) => grant.revokedAt === undefined).map((grant) => grant.workspaceId));
  return (
    <div className="bh-workspace-grants">
      ...
      {error === undefined ? null : <div className="bh-error" role="alert">{error}</div>}
      {grants.filter((grant) => grant.revokedAt === undefined).map((grant) => (
        <div className="bh-workspace-grant" key={grant.id}>
          <div className="bh-workspace-grant-title">{grant.workspaceTitle} <Tag tone="neutral">workspace-write</Tag></div>
          <div className="bh-workspace-grant-path" title={grant.workspacePath}>{grant.workspacePath}</div>
          <Button variant="outline" disabled={busy !== undefined} onClick={() => mutate(grant.id, () => actions.revokeWorkspaceGrant(botSlug, grant.id))}>
            {t('grant.revoke')}
          </Button>
        </div>
      ))}
      {workspaces.filter((workspace) => !activeIds.has(workspace.id)).map((workspace) => (
        <div className="bh-workspace-grant" key={workspace.id}>
          <div className="bh-workspace-grant-title">{workspace.title}</div>
          <div className="bh-workspace-grant-path" title={workspace.path}>{workspace.path}</div>
          <Button variant="outline" disabled={busy !== undefined} onClick={() => mutate(workspace.id, () => actions.createWorkspaceGrant(botSlug, workspace.id))}>
            {t('grant.authorize')}
          </Button>
        </div>
      ))}
      {!loading && workspaces.length === 0 ? <div className="bh-note">{t('grant.noWorkspace')}</div> : null}
    </div>
  );
}

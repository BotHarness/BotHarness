...
| 名册陈列（section 成员/名称/顺序、pins）              | DSH storage 域 **`botharness_roster`**（json 后端，`version 1`、`layout: single`）✅ #66 | global 槽 `{ pins, sectionOrder }` + `sections` 表（key = **主机生成**的 section id）`{ name, channelIds }`；客户端经 `botharness/*` 桥方法读写（写后重拉 `rosterGet`，无乐观状态）；无 storageDomain 的 profile 读写都回 `storage-unavailable`，客户端首屏即渲染只读态、后端可用后重载恢复                                                                                                                  |

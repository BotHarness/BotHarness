...
| 实体                            | 定义                                                               | 关键关系                                                                                             |
| ------------------------------- | ------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------- |
| **PersonaBot**                  | 一等身份：persona、profile、Memory、状态和 Bindings                | 同时最多一个 active Orchestrator，可拥有多个 Work roots（ADR-0016/0024）                             |
...
| **Orchestrator Session**        | PersonaBot 唯一 active 的长期 dispatch root                        | 消费 Bot Inbox，决定回复、创建/复用/停止 Work（ADR-0024/0045）                                       |
| **Work Session**                | 一条工作的 independent DSH root；canonical id = DSH `sessionId`    | 由 Work Directory 寻址；Continuity Key 是 PersonaBot-local alias；DSH Subagent 不算 Work（ADR-0045） |
...
- **Source Event 与 Inbox**：Channel、Bridge、webhook、Work Report 和 system notice 都先成为带可信 provenance 的 immutable Source Event；内容只保存一份。Inbox Trigger 在同一个 Messaging transaction 中建立 PersonaBot-specific Inbox Admission 与 Attention。list 不等于 observed；observed/deferred/ignored/handled 是独立事实（ADR-0025/0036/0037）。
...
- **Orchestrator Session**：每 PersonaBot 同时一个 active root；从 Bot Inbox 接收 attention，决定直接回复、读取/询问已有 Work、创建新 Work 或 defer/ignore。replacement 不改变 PersonaBot 对 Work 的 authority（ADR-0024/0045）。
- **Work Session Directory**：durable、PersonaBot-scoped，按需 `list_work` / `inspect_work`；默认 current-attention、`updatedAt desc, sessionId` 稳定排序，支持 filters、sort、page size 与 opaque cursor。DSH activity/lastRun 与 latest semantic report 分开；内部 Subagents 只聚合展示。
- **Work control tools**：Orchestrator 独占 `create_work`、`send_work_request`、`stop_work` 与查询工具；Work 只拥有 `report_to_orchestrator`。v1 不允许 Work-to-Work direct messaging；Orchestrator 解析 exact target 后中转（ADR-0045）。
- **并发准入**：profile-wide active independent Work 上限默认 3，可由 Human 在全局 Settings 设置 1–32；Orchestrator 只能读取。达到上限时 create/idle-wake 立即返回 stable code、`activeCount`、`limit`、`retryable` 和可读说明；不排队、不写 intent、不创建 Session。降低上限不 cancel 已在运行的 Work。
- **Work delivery**：`context-update` 不 wake；`next-step` 在下一个安全 step 边界 steer；`next-turn` 在本 turn 后 followup。跨 SQLite/DSH 用 stable request/dispatch id 的最小 Work Delivery Intent；bounded reconciliation 后仍歧义则 `needs-repair`，不实现 workflow engine（ADR-0045）。
- **Work 回报与状态**：Agent-authored Work Report 和 Host-authored Lifecycle Notice 都作为不同 provenance 的 Source Event 进入 owning Orchestrator Inbox。milestone、blocked/waiting-human、terminal 触发有意义的 report；未观察的重复可共用 Attention，但 immutable facts 不合并。完整执行历史只在 DSH SessionPersistence。
...
- **归档与停止**：archive 先在事务内关闭新 admissions、wakes、Work requests 与 external actions，再停止 Orchestrator、Work 和 owned Subagents；保留 identity、history 与 audit。reactivate 创建 fresh Orchestrator，不自动 resume 旧执行。
...
| 包                   | 内容                                                                                                                                                  |
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| `@botharness/core`   | Host composition root + deep modules：PersonaBot/ownership、Memory、Messaging、BotWork、Portability、read models；共享 DB owner 不等于共享 repository |
| `@botharness/client` | DSH-native React UI：roster、DM/Channel、Bot Inbox、Work Directory、Settings、recovery/readiness flows                                                |
...
| M3.5 | **安装与 DSH seam 验证门**：真实 bundle/UI 冒烟（#24）+ BotWork runtime seam matrix（#77）                    | 待开始           |
| M4   | **演示闭环**：先交付 DB owner（#79）、Session ownership（#80）、BotWork Runtime（#81），再完成文件研究助手    | 待开始（#11）    |
| M5   | **IM 适配器**：dsh-im 绑定、Lark 国际版验证、客户跟进场景                                                     | 待开始（#12）    |
| —    | **v1.1（post-PoC）**：Messaging、Bot Inbox、Orchestrator/Work UI、Bridge（#30、#46–#48、#75、#78）            | 规划中           |
...
| Orchestrator 中枢成本与单点         | Attention coalescing、digest、按需 Work query；replacement 继承 PersonaBot-scoped authority，不依赖内存句柄                      |
...
**已从 #71 分离的开放设计/验证**：Memory/context delivery 与并发写入（#74）；Inbox/Work/recovery UI（#75）；Profile export/restore UX（#76）；DSH runtime seams（#77）；Feishu provider contract（#78）。具体 page size、sort/filter enum 和性能阈值在实现票中用真实数据确定，不再扩张平台级 grill。

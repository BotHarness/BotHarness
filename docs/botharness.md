...
| 决策记录 | `docs/adr/`（v1.12 落地 0031 三态排序与 scope 内拖拽（#55），手动顺序暂存本地 `roster.json` 的 `channels` 数组序（#66 前）；v1.11 新增 0034 持久化地图与主客分界；v1.10 新增 0031 sidebar 组织与排序、0032 头像与图标；0028 增补 vendored 字形；排序偏好落在 `ui-bot-mode` settings 命名空间，见 #68） |
...
- 迁移：旧 `roster.json` 的排序偏好（全局 `sortMode` + per-section `sortModes[sectionId]`）一次性迁入 `ui-bot-mode` settings ✅ #68，随后从本地记录中清理排序字段、写失败保留源数据；陈列（section 成员/名称/顺序、pins）进 `botharness_roster` 的迁移随 #66。各自只迁移一次。
...
- **排序模式（ADR-0031）**：每个 scope（section / 未分组）三态 `updated`（最新消息在前，按 Channel `updatedAt`）/ `manual`（冻结用户顺序）/ `inherit`（跟随全局默认；section 默认、未分组恒定）；全局默认在头部 `...` 菜单设置；首次手动拖拽切 `manual` 并冻结当前顺序，源 scope 模式不变，「恢复自动」回 `inherit`（拖入触发的切换随 #56）。排序偏好（全局 `sortMode` + per-section `sortModes`）存 Host settings 命名空间 `ui-bot-mode` ✅ #68（`settingsScope` 读写、`settings/document-updated` 刷新），折叠状态留浏览器本地（§2.1、ADR-0034）；per-section 排序行为 ✅ #55（section 内原生 DnD 重排（插入线 + 半行判定，仅真实 drop 提交）、首次拖拽冻结当前顺序并切 `manual`、源 scope 不变、「恢复自动」回 `inherit`；未分组恒随全局；发送消息后按新 `updatedAt` 即时重排，无需刷新；置顶 BOT 网格与 BOT 平铺列表保持手动/名册顺序）；手动顺序暂存本地 `roster.json` 的 section `channels` 数组序（临时缝，随 #66 迁 `botharness_roster`）。行几何对齐原生实测：section 头 34px（`projectRow`）、Channel/session 行 32px、`padding: 0 8px`、行距 2px、区块距 4px、悬停/选中 `--dsw-alias-interactive-bg-hover`、折叠三角 `IconTriangleRightFill14` 旋转 90°、操作字形悬停才显示、Channel 无额外缩进。

...
| M6  | 可备份/可回滚             | 目录可直接复制/打包；git：每 PersonaBot 一个 repo、覆盖记忆目录、每次 turn 一个 commit（摘要 + 来源；人改 source=human）；附件在 repo 外随打包备份                       |
...
| #   | 交付                                                                                                      | 状态         |
| --- | --------------------------------------------------------------------------------------------------------- | ------------ |
| M1  | **BotHarness 骨架**：core 包、PersonaBot registry、bot home（`bot.json` + 记忆目录）、状态事件、设置/存储 | 进行中（#8） |
| M2  | **记忆 MVP**：布局 + front-matter + 目录树注入 + `memory_*` 工具 + 可见性过滤 + 原子写                    | 待开始       |
| M3  | **Roster 与委派**：client 包、`main` 面板（名册/详情/新建）、@委派、工位会话、六态展示                    | 待开始       |
| M4  | **演示闭环**：文件研究助手（workspace 绑定、执行、汇报、waiting/审批）                                    | 待开始       |
| M5  | **IM 适配器**：dsh-im 绑定、Lark 国际版验证、客户跟进场景                                                 | 待开始       |

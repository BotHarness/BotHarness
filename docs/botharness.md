...
| M6  | 可备份/可回滚             | 目录可直接复制/打包；git：每 PersonaBot 一个 repo、单分支、覆盖记忆目录，每次记忆写入一个 commit（`summary` + 来源即 message；人改 source=human；同一 turn 多次写入按写入逐个提交）；附件在 repo 外随打包备份；未来 UI 管历史（clone + reset 到版本） |
...
| M2  | **记忆 MVP**：布局 + front-matter + 目录树注入 + `memory_*` 工具 + 可见性过滤 + 原子写                    | 已完成（#9）     |

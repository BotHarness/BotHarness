...
- **Resource bounds are part of the contract, not a suggestion.** The container runs with `--memory 2g --memory-swap 2g` (swap disabled, so memory cannot silently become disk), `--shm-size 512m` (Chromium's shared memory is charged to the container), `--pids-limit 4096`, and `--cpus 2`; the idle policy stops it after 30 minutes of no viewers. Measured resident memory for the resting desktop fell from ~2.4 GiB (4 GiB limit, 1 GiB shm) to ~1.15 GiB of a 2 GiB limit.
...
- Resource limits are configuration, not code: `memory`, `shmSize`, `pidsLimit`, `cpus`, and `idleStopMinutes` remain overridable per Host.

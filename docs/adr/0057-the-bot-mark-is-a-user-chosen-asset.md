...
- **The Settings navigation is tagged, not extended.** DSH's shell picks a nav glyph from a hardcoded map keyed by section id (unknown ids fall back to the settings gear), the `settings.section` registration carries no icon option, and `SlotLabel` accepts only strings — there is no seam. The client therefore finds the Bot section's nav cell by its localized label (never by position), hides the shell's glyph, and inserts the chosen mark — hiding and sizing go through inline styles, because the shell re-renders the button's `className` (which would drop a marker class) while React never touches inline styles it did not set. A MutationObserver re-applies after shell re-renders and on preference or palette changes, and a cell that cannot be matched simply keeps the gear.

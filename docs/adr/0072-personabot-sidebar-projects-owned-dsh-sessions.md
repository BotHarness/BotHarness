...
The right pane of a Human–PersonaBot DM presents **Sessions / 会话**, not a separate Assignment task list. It is a read model over independent DSH root Sessions. BotHarness Session Ownership determines which Session IDs belong to the PersonaBot and whether each root is Orchestrator or Assignment. DSH's Client Session catalog supplies the display title, workspace cwd, update time, and live running state. Clicking a row uses native `UiWorkspace.openSession` for that exact ID. DSH Subagent children are excluded. A cwd match never establishes Bot ownership. The native Session header contributes a return action through DSH's `conversation.session.header.actions` slot: a Host reverse lookup resolves the current root Session's owner, and only an existing owned Orchestrator or Assignment offers a return action with that PersonaBot's avatar, navigating back to its Human DM. Subagents and unrelated native Sessions have no return action.

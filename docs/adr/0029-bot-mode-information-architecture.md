...
A PersonaBot DM alone shows **PersonaBot navigation** in the right-side shell area. Chat and Memory are its primary destinations: Chat renders the DM history and composer, while Memory replaces the center body with the PersonaBot's file-backed Memory surface. Owned Assignments appear directly below those destinations as a subordinate list, not behind a top-level Assignments destination. The list projects purpose, state, recent activity, and latest report from Assignment Sessions; it never shows the Orchestrator Session. Selecting an Assignment opens a read-only Assignment detail surface, with raw DSH Session content available only through an explicit secondary action. A group Channel has no PersonaBot navigation because it has no single owning PersonaBot.

Memory in the DM is the ordinary Human entry for browsing, editing, and inspecting Git history. Settings retains administration, diagnostics, and links into that surface rather than a second full editor. The first tracer bullet is files-first: select a PersonaBot DM, open Memory, browse and read a file, edit and save it, then inspect the resulting commit history and diff. Graph, activity heatmaps, Shared Memory, and other derived views are deferred until the file workflow is proven.
...
- Memory becomes an ordinary PersonaBot surface; Settings remains the administrative and diagnostic entry.
- The first UI tracer bullet delivers Chat plus the files-first Memory loop. The Assignment list and detail surface follow the Assignment read model instead of blocking that slice.

...
The working tree changes as soon as Git switches. Ordinary file tools in the same Session therefore read the new branch's files; a missing file is an ordinary missing-file result. The Session-frozen Persona prompt remains unchanged under ADR-0060. Uncommitted changes block this initial switch operation without a force reset; Orchestrator coordination of conflicts and branch creation are separate follow-up slices in [#263](https://github.com/BotHarness/BotHarness/issues/263) and [#262](https://github.com/BotHarness/BotHarness/issues/262).

...
- 明确 BotWork control plane：每 PersonaBot 一个 active Orchestrator；independent Work root 以 DSH `sessionId` 寻址；durable Work Directory；五个 Orchestrator tools；Work-only `report_to_orchestrator`；无 Work-to-Work 直连；全局 active Work 上限默认 3，由 Human 设置，超限立即结构化失败且不排队。
...
- 重写 living architecture 图，对齐 platform spec v1.12 与 app PRD v1.5。后续拆为 #74–#82 并设置 GitHub 原生 blockers；#55 仍可与新的 Inbox/Work UI design 并行。
- 重构可安装的 DSH skill：先进入规范 Context 与 Decision Tree，再按需下钻 Host/Client/Slots；新增 Bot runtime 参考，明确区分产品 IM、Work ownership 与 DSH Subagent delegation。

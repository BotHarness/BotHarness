...
- Defined the Assignment control plane: one active Orchestrator per PersonaBot, independent Assignment Sessions keyed by DSH `sessionId`, a durable Assignment Directory, five Orchestrator tools, Assignment-only `report_to_orchestrator`, no direct Assignment-to-Assignment messaging, and a Human-owned global active-Assignment limit of 3 by default with immediate structured failure and no queue.
...
- Rewrote the living architecture diagrams and aligned platform spec v1.12 and app PRD v1.5. Follow-up work is split into #74–#82 with native GitHub blockers; #55 remains parallel to the new Inbox/Assignment UI design.
- Refactored the installable DSH skill around a canonical Context and Decision Tree before Host/Client/Slots details, and added a Bot runtime reference that keeps product IM, Assignment ownership, and DSH Subagent delegation distinct.

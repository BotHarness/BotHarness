...
| **审计 SessionEvent + turn 生命周期清理**  | `PerryLink`（`dsh-click/observed                                                                                                                                                       | action` + known-type 门控）；`AzaiSakura`/`wushi2333`（`turn/end` 释放 daemon session/overlay）     | "turn 结束 → 释放观察 lease / 光标 overlay / VNC 写入权" |

...
1. 任务书假定存在 `docs/research/2026-09-17-rakazo-architecture.md`（rakazo 报告）与 `reference/rakazo` 克隆；**实际两者都不存在**（本轮开始时 `docs/research/` 目录不存在，`reference/` 只有新克隆的 openmausbot）。我已按任务书给出的 pin SHA 自行克隆 rakazo，本报告的所有 rakazo 结论直接来自该 clone 的一手文件，**不依赖该缺失报告**；若该报告后来出现，应视为独立材料并复核其 SHA。
...
- **缺失材料**：任务书引用的 `docs/research/2026-09-17-rakazo-architecture.md` 在仓库中不存在；本报告未使用它，也无法核对其 SHA 是否与 `83698c0de8eefab5993a84ed76cb81674a04ea32` 一致。

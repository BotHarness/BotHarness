...
| # | 规则 | 说明 |
|---|---|---|
| M1 | 文件是唯一事实源 | 不引入私有格式；Markdown + front-matter（可选 YAML：tags、updated_at、sources） |
| M2 | 原子写 | 写临时文件 + `rename` 原子替换；禁止原地截断；同一 Bot 串行写（单写者） |
| M3 | **目录树进上下文** + 按需检索 | turn 注入目录树（文件清单 + 一句话摘要 + 更新时间，目标 < 2k tokens）；细节用 `memory_search`（ripgrep）/ 读文件 |
...
1. 记忆的 **front-matter schema**：是否强制 `tags / updated_at / sources` 字段（布局已定：`MEMORY.md` + 主题目录 + 目录树注入）？（M3 决策）
...
4. 目录树注入的**规模阈值与折叠策略**：文件数/字符数到多少时改为只注入 `MEMORY.md` + 按需检索？（M3 决策）

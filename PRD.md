...
> 作为用户，我 @ 一个 PersonaBot 交付工作，它在后台做完并汇报；可以同时交给它几件事。

- AC-2.1 composer `@PersonaBot` 可唤起（`/bot` 命令兜底）。
- AC-2.2 委派 = 在目标 Session 起一轮；工位空闲唤醒，忙则开子会话。
- AC-2.3 状态实时反映到 roster（thinking/working/waiting/blocked/done）。
- AC-2.4 完成后汇报（会话内 + 名册「最近汇报」）。
...
- AC-4.1 绑定 dsh-im 的 Bot/凭据（**domain=lark 优先验证**）。
- AC-4.2 群/话题路由复用基座；DM 一等；记忆与 sidebar 同一份。
- AC-4.3 入站文件「引用即提升」为稳定路径。
- AC-4.4 客户跟进北极星：群里聊客户 → 档案持续更新 → 换群/换 thread 引用同一档案。

### US-5 审批与安全（P0）

- AC-5.1 外部副作用 → `waiting`，确认后执行。
...
| 类别   | 要求                                                                                            |
| ------ | ----------------------------------------------------------------------------------------------- |
| 安全   | 最小权限（附录 A）；审批分级；沙箱隔离执行；凭据仅在 credentials 服务；DM 内容按可见性披露      |
...
| #   | 应用侧交付                                    |
| --- | --------------------------------------------- |
| M3  | Roster 与委派（client 包；应用主战场）        |
| M4  | 文件研究助手演示闭环                          |
| M5  | IM 适配器（dsh-im 绑定、Lark 验证、客户跟进） |
...
| 跨 PersonaBot 通信    | seam 已留（registry + 事件）         | 单 Bot 闭环成立后           |
...
- 技能初始化（对应 `setup-matt-pocock-skills`）：
  ...
  - 领域文档：单 context；根 `CONTEXT.md` + `docs/adr/`（18 篇）——见 `docs/agents/domain.md`

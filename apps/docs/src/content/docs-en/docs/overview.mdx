...
- [Quickstart](/en/docs/quickstart): run it locally, directory layout, common commands
- [Architecture & data flow](/en/dev/architecture): system context, modules, data flow, boundaries (kept current)
- [Platform spec](/en/dev/spec/platform): PersonaBot, memory rules, state, and how work happens
- [App PRD](/en/dev/spec/app-prd): DeepSeekBot user stories and acceptance criteria
- [Domain glossary](/en/dev/spec/context): canonical usage of terms (PersonaBot / Agent / Session / Workspace)
- [Decision records](/en/dev/adr/0015-botharness-is-a-dsh-plugin-layer): why things are the way they are
- [Changelog](/en/changelog): what changed in each version

...
- [快速开始](/quickstart)：本地跑起来、目录结构、常用命令
- [架构与数据流](/architecture)：系统上下文、模块、数据流、边界（持续维护）
- [平台规格](/spec/platform)：PersonaBot、记忆规则、状态与工作方式
- [应用 PRD](/spec/app-prd)：DeepSeekBot 的用户故事与验收标准
- [领域词表](/spec/context)：术语的规范用法（PersonaBot / Agent / Session / Workspace）
- [决策记录](/adr/0015-botharness-is-a-dsh-plugin-layer)：为什么这么做

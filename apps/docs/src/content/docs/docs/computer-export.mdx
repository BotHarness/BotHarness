...
- To import, **Choose archive file…** picks a local `.tar`, then **Authorize and import** streams it up and runs the standard import. Long archive names wrap with an ellipsis; the full name is one hover away.

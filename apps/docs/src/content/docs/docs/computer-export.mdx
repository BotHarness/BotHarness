...
- The **export directory** is a path on the *host*, configured in **Settings → BotHarness → Computer**. The archive file appears there; import only lists and accepts files from that directory.

## Export

1. Open **Settings → BotHarness → Computer**. Under _Export / import_, pick an export directory if none is set — export and import stay disabled until one is.
...
3. When it finishes, the note shows the archive path — one timestamped `.tar` file.
...
3. In **Settings → BotHarness → Computer**, set an export directory on the target and put the archive in it.
